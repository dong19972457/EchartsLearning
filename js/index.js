...
(function() {
    ...
    $(".order").on("click", ".filter a", function() {
        ...
        // 显示内容切换
        var index = $(this).index();
        $(".order h4:eq(0)").html(data['day' + index].orders);
        $(".order h4:eq(1)").html(data['day' + index].amount);
    })
    var index = 0;
...
(function() {
    ...
    $(".sales").on("click", ".caption a", function() {
        // 保证点击后 index为当前被点击a的索引号
        // 否则，在往回点击a时，鼠标离开后轮播会直接跳到点击前的下一个
        index = $(this).index() - 1;
        $(this).addClass("active").siblings().removeClass("active");
        // 拿到自定义属性 data-type
        // 即a的data-type,通过dataset获取
...
(function() {
    ...
    function render(that) {
        ...
        $.each(hotData[that.index()].brands, function(index, item) {
            subHTML += `<li><span>${item.name}</span>
